...
(10,'M','2016-02-22',10,'Omsk','2019-12-22'), 
(11,'W','2015-03-05',11,'Tutuevo','2019-12-22')
;
...
(11,10,2,'Hi, User 2','2019-12-26'),
(12,10,2,'Hi, User 2','2019-12-27'),
(13,10,2,'Hi, User 2','2019-12-28'),
(14,5,2,'Hi, User 2','2019-12-22'),
(15,5,2,'Hi, User 2','2019-12-22'),
(16,3,2,'Hi, User 2','2019-12-22')
;

insert into friend_requests
values
(2,3,'requested','2019-12-22','2019-12-23'),
(5,2,'requested','2019-12-22','2019-12-23'),
...
(2,4,'approved','2019-12-22','2019-12-23'),
...
(10,2,'requested','2019-12-23','2019-12-23'),
(2,10,'approved','2019-12-24','2019-12-24'),
(2,5,'approved','2019-12-24','2019-12-24')
;
...
insert into likes
values
(2,2,21,'2019-12-22',1,2),
(3,3,22,'2019-12-22',2,10),
(4,4,23,'2019-12-22',3,3),
(5,5,24,'2019-12-22',4,4),
(6,6,25,'2019-12-22',5,5),
(7,7,26,'2019-12-22',6,6),
(8,8,27,'2019-12-22',7,7),
(9,9,28,'2019-12-22',8,6),
(10,10,29,'2019-12-22',9,5),
(11,11,10,'2019-12-22',10,4)
;
...
select tab1.from_user_id, max(messages_amount) from
	(select messages.from_user_id, count(messages.from_user_id) as messages_amount, friend_requests.status
		from messages 
		join friend_requests on friend_requests.target_user_id = messages.from_user_id
			where messages.to_user_id = 2
		 	and friend_requests.status = 'approved'
		group by messages.from_user_id 
		order by count(messages.from_user_id) desc) as tab1;


-- 2. Подсчитать общее количество лайков, которые получили пользователи младше 10 лет..


-- в исходном коде выше изменил таблицу likes: добавил поле, которое будет определять кому и от кого пришел лайк (to_user_id, from_user_id)
-- наполнил таблицу новыми данными

-- отбираем пользователей младше 10 лет
select user_id, birthday, (2020 - year(birthday)) as age from profiles
where (2020 - year(birthday)) < 10;

-- смотрим сколько на них прилетело лайков
select to_user_id, count(from_user_id) from likes
group by to_user_id;

-- теперь смотрим сколько всего получили лайков пользователи младше 10 лет

select profiles.user_id, profiles.birthday, (2020 - year(profiles.birthday)) as age, tab1.amount_of_likes
	from (select to_user_id, count(from_user_id) as amount_of_likes from likes
group by to_user_id) as tab1
	join profiles on profiles.user_id = tab1.to_user_id
where (2020 - year(profiles.birthday)) < 10;

-- общая сумма лайков:

select sum(tab1.amount_of_likes)
	from 
	(select to_user_id, count(from_user_id) as amount_of_likes from likes group by to_user_id) as tab1
	join profiles on profiles.user_id = tab1.to_user_id
		where (2020 - year(profiles.birthday)) < 10;
...
-- добавляем к предыдущей таблице поле с полом отправителя лайков
select profiles.user_id, profiles.birthday, (2020 - year(profiles.birthday)) as age, tab1.amount_of_likes, tab1.from_user_gender
	from (select likes.to_user_id, count(likes.from_user_id) as amount_of_likes, profiles.gender as from_user_gender 
		from likes
		join profiles on profiles.user_id = likes.from_user_id
		group by likes.to_user_id, profiles.gender) as tab1
	join profiles on profiles.user_id = tab1.to_user_id
where (2020 - year(profiles.birthday)) < 10;

-- считаем сколько поставило лайков мужчин (ответ - 1)

select count(tab1.from_user_gender) as count_male
	from
		(select profiles.user_id, profiles.birthday, (2020 - year(profiles.birthday)) as age, tab1.amount_of_likes, tab1.from_user_gender
			from (select likes.to_user_id, count(likes.from_user_id) as amount_of_likes, profiles.gender as from_user_gender 
			from likes
	...
	join profiles on profiles.user_id = tab1.to_user_id
where 
(2020 - year(profiles.birthday)) < 10 
and tab1.from_user_gender = 'M') as tab1

-- считаем сколько поставило лайков женщин (ответ - 1)

select count(tab1.from_user_gender) as count_female
	from
		(select profiles.user_id, profiles.birthday, (2020 - year(profiles.birthday)) as age, tab1.amount_of_likes, tab1.from_user_gender
			from (select likes.to_user_id, count(likes.from_user_id) as amount_of_likes, profiles.gender as from_user_gender 
			from likes
...
where 
(2020 - year(profiles.birthday)) < 10 
and tab1.from_user_gender = 'W') as tab1

-- итого - поровну. 
-- как сцепить это в одну таблицу и красиво вывести в цельный результат не придумал. :(